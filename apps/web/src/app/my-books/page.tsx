...
export default function MyBooksPage() {
    ...
    const fetchMyBorrows = useCallback(async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                setError(res.data.message || "Failed to load borrowings");
            }
        } catch (err: any) {
            setError(err.response?.data?.message || err.message || "Failed to load borrowings");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    // Filter to active (PENDING & APPROVED)
    const activeRecords = borrows.filter(b => b.status === "PENDING" || b.status === "APPROVED");

    // Calculate metrics
    const stats = {
        active: activeRecords.filter(b => b.status === "APPROVED").reduce((sum, b) => sum + b.borrowBooks.length, 0),
        pending: activeRecords.filter(b => b.status === "PENDING").length,
        overdue: activeRecords.filter(b => {
            if (b.status !== "APPROVED" || !b.returnDate) return false;
    ...
    const calculateEstimatedPenalty = (dueDateStr: string | null) => {
        ...
        if (days <= 0) return 0;
        return days * 50; // ₹50 per day late penalty
    };
